...
try {
 ...
 await page.getByLabel('Shorter session: hide optional accessories',{exact:true}).check();
 ...
 await page.getByText('Session options',{exact:true}).click();
 assert.ok(await page.getByLabel('Shorter session: hide optional accessories',{exact:true}).isChecked());
 ...
 assert.equal(strength.sessionMode,'shorter');assert.equal(strength.linkedExposureIds.length,1);
