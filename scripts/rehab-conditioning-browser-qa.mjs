...
try {
 ...
 assert.equal(await page.getByRole('link',{name:'Short Demo',exact:true}).count(),7);
 assert.equal(await page.getByRole('button',{name:/^View illustration for /}).count(),7);
 await page.getByLabel('Session block',{exact:true}).selectOption('2. Leg control circuit');
 ...
 await page.getByLabel('Step-Up set 2 reps',{exact:true}).fill('8');
 ...
 await page.getByLabel('Session block',{exact:true}).selectOption('4. Conditioning');
