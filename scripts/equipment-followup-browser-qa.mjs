...
try {
 ...
 assert.equal(await card.getByRole('link',{name:'Short Demo: Sled push',exact:true}).getAttribute('href'),'https://www.youtube.com/watch?v=QwscR2BhdEg');await card.getByText('Reference only · Demo does not unlock this exercise.',{exact:true}).waitFor();
 await page.screenshot({path:resolve(artifacts,"equipment-followup-sled.png"),fullPage:true});
 pass("Canonical sled reference has a reviewed demo and retains visible reference-only status");
 ...
 await search.fill('tibialis');await page.getByRole('heading',{name:'Tibialis raise',exact:true}).waitFor();
 assert.equal(await page.getByRole('link',{name:'Short Demo: Tibialis raise',exact:true}).getAttribute('href'),'https://www.youtube.com/watch?v=nQKgHwi8W9E');
 await page.getByText('Reference only · Demo does not unlock this exercise.',{exact:true}).waitFor();
 await page.getByRole('button',{name:'View illustration for Tibialis raise',exact:true}).click();
 const illustration=page.getByRole('dialog',{name:'Tibialis raise illustration',exact:true});
 await illustration.waitFor();await illustration.locator('img').evaluate(img=>img.decode());
 assert.equal(await illustration.locator('img').evaluate(img=>img.naturalWidth),640);
 await page.screenshot({path:resolve(artifacts,'research-library-illustration.png'),fullPage:true});
 await page.getByRole('button',{name:'Close illustration',exact:true}).click();
 await page.getByRole('button',{name:'Favorite: Tibialis raise',exact:true}).click();
 await page.waitForFunction(()=>document.querySelector('[aria-label="Favorite: Tibialis raise"]')?.getAttribute('aria-pressed')==='true');
 pass('Research exercise exposes reviewed demo and working illustration without unlocking programming');
 await search.fill('belt-squat isometric');await page.getByText('Equipment setup review required before use.',{exact:true}).waitFor();
 assert.equal(await page.getByRole('link',{name:'Short Demo: Belt-squat isometric',exact:true}).count(),0);
 await page.getByRole('link',{name:'Related demo: Belt-squat isometric',exact:true}).waitFor();
 await page.getByText(/Different belt-squat machine. Shows a squat hold/).waitFor();
 await page.getByRole('button',{name:'View illustration for Belt-squat isometric',exact:true}).click();
 await page.getByText('Movement concept only; exact attachment/setup review is still required.',{exact:true}).waitFor();
 await page.getByRole('button',{name:'Close illustration',exact:true}).click();
 pass('Belt-squat concept retains visible unresolved setup status');
 const favorite=await page.evaluate(async()=>{const db=await new Promise(r=>{const q=indexedDB.open('achilles-return-db');q.onsuccess=()=>r(q.result);});return new Promise(r=>{const q=db.transaction('settings').objectStore('settings').get('exercise-favorites');q.onsuccess=()=>{db.close();r(q.result);};});});
 assert(favorite.ids.includes('library-tibialis-raise'));
 pass('Research library favorites persist in IndexedDB');
