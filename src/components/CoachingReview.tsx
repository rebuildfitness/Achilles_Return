...
export function CoachingReview({ session, sessions, expanded = false }: {session: Session; sessions: Session[]; expanded?: boolean}) {
 ...
 return <><details className="coaching-review" open={expanded || undefined}><summary>AI coaching review</summary><p>{session.nextDayResponse ? "Updated report: next-morning feedback is included." : "Your report is ready now. Next-morning feedback is pending; add it later and export an updated report."}</p><p>Review before sharing. This includes your workout and rehabilitation information. Copy or attach it to your chosen AI conversation; nothing is sent automatically.</p><div className="exercise-actions"><button className="text-button" onClick={copy}>Copy coaching report</button><button className="text-button" onClick={download}>Download Markdown</button>{typeof navigator.share==='function' && <button className="text-button" onClick={share}>Share report</button>}</div><p role="status">{message}</p><details><summary>Preview report</summary><label>Coaching report<textarea aria-label="Coaching report" readOnly rows={12} value={report} /></label></details></details></>;
}
